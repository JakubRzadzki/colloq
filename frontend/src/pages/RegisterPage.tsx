...
export function RegisterPage({ t, lang }: { t: any, lang: string }) {
  ...
  const handleRegister = async (e: any) => {
    e.preventDefault();
    if (!captchaToken) return alert("Captcha!");
    try {
      await axios.post(`${API_URL}/register`, {
        user: { email: e.target.email.value, password: e.target.password.value, university_id: e.target.university_id.value },
        captcha_token: captchaToken
      });
      alert(t.successReg);
      navigate('/login');
    } catch { alert(t.errorReg); setCaptchaToken(null); }
  };
          ...
          <form onSubmit={handleRegister} className="form-control flex flex-col gap-4">
            ...
            <div>
               ...
               <div className="join w-full mb-2">
                 <div className="btn btn-square join-item no-animation"><Search size={18}/></div>
                 <input
                    type="text"
            ...
            <div className="flex justify-center my-4 p-2 bg-base-200 rounded-lg">
              <Turnstile siteKey="1x00000000000000000000AB" onSuccess={setCaptchaToken} />
            </div>
            <button className="btn btn-primary w-full text-white text-lg">{t.registerBtn}</button>
          </form>
